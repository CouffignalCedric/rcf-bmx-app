    ...
    Papa.parse(SHEET_URL, {
      ...
      complete: (results) => {
        const normalizedData = results.data.map(row => {
          const newRow = {};
          // Normalise les clés du CSV (minuscules et sans espaces)
          ...
          <div className="m-modal-drawer" onClick={e => e.stopPropagation()}>
            ...
            <div className="m-modal-map">
              <iframe
                ...
                height="180"
                style={{ border: 0 }}
                /* Ajout de BMX devant la localisation pour forcer la recherche sur la piste */
                src={`https://maps.google.com/maps?q=BMX%20${encodeURIComponent(selectedRace.location)}&t=&z=13&ie=UTF8&iwloc=&output=embed`}
              >
              </iframe>
            </div>

            <div className="m-modal-grid">
              {/* Lien Maps optimisé pour la recherche de point d'intérêt */}
              <a
                href={`https://www.google.com/maps/search/?api=1&query=piste+bmx+${encodeURIComponent(selectedRace.location)}`}
                target="_blank"
                rel="noreferrer"
                className="m-action-btn maps"
              >
                Maps
              </a>

              {/* Waze reste identique, il est déjà bien paramétré */}
              <a
                href={`https://waze.com/ul?q=piste+bmx+${encodeURIComponent(selectedRace.location)}&navigate=yes`}
                target="_blank"
                rel="noreferrer"
                className="m-action-btn waze"
              >
                Waze
              </a>

              <a
                href={`https://www.airbnb.fr/s/${encodeURIComponent(selectedRace.location)}/homes`}
                target="_blank"
                rel="noreferrer"
                className="m-action-btn dodo"
              >
                Dodo
              </a>
            </div>
